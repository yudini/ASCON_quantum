...
def Reverse(eng,x0,x1,x2,x3,x4):
    # x1
    for i in range(64):
        Toffoli_gate(eng, x2[i], x3[i], x1[i])
    ...
    for i in range(64):
        CNOT | (x4[i], x0[i])
        CNOT | (x0[i], x1[i])
        Toffoli_gate(eng, x1[i], x2[i], x0[i])
        X | x1[i]
        X | x0[i]

    #x4
    for i in range(64):
        Toffoli_gate(eng, x0[i], x1[i], x4[i])
        X | x0[i]
...
def Substitution_LinearDiffusion_Layer(eng, x0,x1, x2, x3, x4, pa, pb):
    for i in range(pa+pb):
        ...
        for i in range(64):
            X | x3[i]
        for i in range(64):
            Toffoli_gate(eng, x3[i], x4[i], x2[i])
        for i in range(64):
            CNOT | (x2[i], x3[i])
        for i in range(64):
            X | x2[i]
        ...
        for i in range(64):
            ...
            X | x4[i]
        for i in range(64):
            Toffoli_gate(eng,x4[i],x0[i],x3[i])
        for i in range(64):
            CNOT | (x3[i], x_3[i])
