...
def Substitution_Layer(eng,x0,x1,x2,x3,x4):
    ...
    for i in range(64):
        Toffoli_gate(eng, new_x1[i], x2[i], x0[i])
    for i in range(64):
        Toffoli_gate(eng, new_x2[i], x3[i], x1[i])

    for i in range(64):
        Toffoli_gate(eng, new_x3[i], x4[i], x2[i])
        X | new_x1[i]

    for i in range(64):
        Toffoli_gate(eng, new_x0[i], new_x1[i], x4[i])
        X | new_x0[i]

    for i in range(64):
        Toffoli_gate(eng, new_x4[i], new_x0[i], x3[i])
...
def Permutation_a(eng,pa,x0,x1,x2,x3,x4):
    # S = (MSB) x0  ||  x1  ||  x2  ||  x3  || x4 (LSB)
    #       256:320   192:256 128:192  64:128   0:64

    for i in range(pa):
        add_constant(eng,x2,i)
        Substitution_Layer(eng,x0,x1,x2,x3,x4)    #correct
        LinearDiffusion_Layer(eng,x0,x1,x2,x3,x4)

def Permutation_b(eng,pb,x0,x1,x2,x3,x4):
    for i in range(pb):
        add_constant(eng,x2,i+pb)
        Substitution_Layer(eng, x0, x1, x2, x3, x4)
        LinearDiffusion_Layer(eng, x0, x1, x2, x3, x4)


def Initialization(eng,pa,x0,x1,x2,x3,x4,key):
    Permutation_a(eng,pa,x0,x1,x2,x3,x4)
...
def Associated(eng,pb,A,A_len,x0,x1,x2,x3,x4):
    for i in range(32):
        CNOT | (A[i],x0[32+i])

    X | x0[31]

    Permutation_b(eng,pb,x0,x1,x2,x3,x4)
...
def Finalization(eng,pa,x0,x1,x2,x3,x4,key):
    for i in range(64):
        CNOT | (key[i + 64], x1[i])
        CNOT | (key[i], x2[i])
...
def main(eng):
    ...
    Initialization(eng,pa,S[256:320],S[192:256],S[128:192],S[64:128],S[0:64],key)   # end
    ...
    Finalization(eng,pa,S[256:320],S[192:256],S[128:192],S[64:128],S[0:64],key)
